...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
The loss matrix will make \textit{false negatives} be classified much harsher compared to \textit{false positives}.
...
\end{verbatim}
The result indicates a misclassification rate 0.12-0.15 worse than unweighted. If the loss matrix was flipped it would result in an improvement in performance of about 2-3 percent compared to the \textit{naive bayesian} model without loss matrix and about the same as the best decision tree classifier.
